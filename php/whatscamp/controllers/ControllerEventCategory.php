...
{ 
    ...
    {
        $stmt = $this->pdo->prepare('UPDATE  tbl_whatscamp_event_categories 
                                        SET is_deleted = 0
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO tbl_whatscamp_event_categories( 
                                            category_id,
                                            event_id,
                                            created_at,
                                            updated_at ) 
    ...
    public function getCategoryEventByCategoryIdAndEventId($category_id, $event_id) 
    {
        $stmt = $this->pdo->prepare('SELECT * 
                                        FROM tbl_whatscamp_event_categories 
                                        WHERE category_id = :category_id AND event_id = :event_id');
