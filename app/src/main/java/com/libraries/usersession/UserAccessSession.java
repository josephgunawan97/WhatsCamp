...
public class UserAccessSession {
	...
	private static final String GOOGLE_ID 		= "sekln0LDNKANWdsee";
	...
	private static final String FILTER_RADIUS_HOME = "FILTER_RADIUS_HOME";
	...
	private static final String SHOW_WALKTHROUGH 		= "wefkwnr3nkl30002";
	...
	public Boolean isLoggedInFromSocial() {
		...
		String googleId = sharedPref.getString(GOOGLE_ID, null);
		boolean isSocial = ( (facebookId != null && facebookId.length() != 0) || 
								(twitterId != null && twitterId.length() != 0) ||
				(googleId != null && googleId.length() != 0) ) ? true : false;
	...
	public float getFilterDistance() {
		float distance = sharedPref.getFloat(FILTER_RADIUS_DISTANCE, 0);
	...
	public void setHomeRadius(int radius) {
		editor.putInt(FILTER_RADIUS_HOME, radius);
		editor.commit();
	}

	public int getHomeRadius() {
		int distance = sharedPref.getInt(FILTER_RADIUS_HOME, 0);
		return distance;
	}
	public void setWillNotShowWalkthrough(boolean isShown) {
		editor.putBoolean(SHOW_WALKTHROUGH, isShown);
		editor.commit();
	}

	public Boolean willNotShowWalkthrough() {
		if(sharedPref == null)
			return false;

		return sharedPref.getBoolean(SHOW_WALKTHROUGH, false);
	}
}
